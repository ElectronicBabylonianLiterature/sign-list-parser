...
const aslSemantics = aslGrammar.createSemantics().addOperation('eval', {
  ...
  Instance: function (citation, equals, cdlLabel, comma, word) {
    return {
      citation: _.head(citation.eval()),
      cdlLabel: cdlLabel.eval(),
      word: _.head(word.eval())
    }
  },
  CdlLabel: function (project, colon, number, line) {
    return {
      project: project.sourceString,
      number: number.sourceString,
      line: line.sourceString
    }
  },
  Phrase: function (alnum) {
